...
const allCompanies = async (req, res) => {
    ...
    try {
        ...
        if (allCompany) {
            ...
            res.status(200).json("No companies registered yet")
        }
    }
    catch (err) {
        res.status(401).json("Company get api is not working")
    }
}
...
const allUsers = async (req, res) => {
    ...
    try {
        ...
        if (allUser) {
            ...
            res.status(200).json("No User registered yet")
        }
    }
    catch (err) {
        res.status(401).json("User get api is not working")
    }
}
...
const getOneJob = async (req, res) => {
    ...
    const data = await jobPost.findOne({ _id: id })
    if (data) {
        res.status(200).json({
            message: data,
            status: true,
...
const getUserDetails = async (req, res) => {
    const uid = req.payload; // Assuming the user ID is stored in req.user
// console.log(uid);
    try {
...
module.exports = {
    ...
    usersData, companyData, deleteCompany, deleteUser, allCompanyJob, allLimitJobs,getOneJob,
    getUserDetails,allCompanies,allUsers
}